...
- **前端**:`InsightCard` 加「整體建議」敘述區塊;新增 `EfficiencyGapChart`(效率落差分歧長條,`efficiency_gap×100` 轉百分點 pp,正綠負紅 + `ReferenceLine x=0`)。
...
**版面更新(BLUF,取代原「上圖表、下報告」順序)**:策略卡片改為「**整體建議 → ROAS/CPA 圖 → 效率落差圖 → 各來源建議**」(結論先行 Bottom Line Up Front:先給 TL;DR,再給數據佐證,最後給各家可執行動作)。
- `InsightCard` 加 `section?: 'summary' | 'details'` prop:`summary` 只渲染整體建議(放圖上方)、`details` 只渲染逐條(放圖下方,並加「各來源建議」標題);空狀態/降級訊息只在 `details` 出現一次,避免重複。省略 prop 維持全渲染(向後相容)。
- 取代 R13「上圖表、下報告(先數據後結論)」:整體建議是月度策略卡片的執行摘要,置頂較利快速掃讀;個別建議仍在數據之後。
